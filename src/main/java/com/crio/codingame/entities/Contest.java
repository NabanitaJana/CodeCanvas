...
public class Contest extends BaseEntity{
    ...
    private void validateQuestionList(List<Question> qList, Level contestLevel) throws InvalidContestException {
        if(qList.isEmpty()){
            throw new InvalidContestException("Question list empty");
        }
        Level firstlevel=qList.get(0).getLevel();
        if(!qList.stream().allMatch(question -> question.getLevel().equals(firstlevel))){
            throw new InvalidContestException("All question must be of same level");
        }
        if(!firstlevel.equals(contestLevel)){
            throw new InvalidContestException("contest level must be equal to all questions level");
        }
